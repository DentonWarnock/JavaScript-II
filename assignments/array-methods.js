...
// so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L 
// and log the result
let largeShirts = [];
...
ticketPriceTotal = runners.reduce((total, people) => {
...
// Problem 1  - to help with day of registration a list of last names in alphabetical order is needed
let lastNames = [];
runners.forEach((person) => {
  return lastNames.push(person.last_name);
})
lastNames.sort();
console.log(lastNames);


// Problem 2 - we need a list of first names and emails to send out a reminder to sign up for next year's race.
let emailList = runners.map(people => {
  return {
    first_name: people.first_name,
    email: people.email
  };
});
console.log(emailList); //working!

// Problem 3 - we want to create donation amount tiers to know who donated less than $100, from $100-$199, and greater than $200 to recognize them in 
// 
lessThanOneHundred = runners.filter(people => people.donation > 0 && people.donation < 100); // < $100
oneToTwoHundred = runners.filter(people => people.donation >= 100 && people.donation < 201); // $100 - $200
moreThanTwoHundred = runners.filter(people => people.donation > 200); // > $200

// console.log(lessThanOneHundred);
// console.log (oneToTwoHundred);
// console.log (moreThanTwoHundred);

//TODO: Callback funtion below showing as undefined. 
function donationAmounts(tier, cb) {  
  return cb(tier)
};
...
console.log(nameTransfer(oneToTwoHundred));
console.log(nameTransfer(moreThanTwoHundred));
donationAmounts(lessThanOneHundred, nameTransfer);
